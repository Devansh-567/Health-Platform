...
const createSchema = z.object({
  ...
  // Needed for ambulance dispatch (route origin/destination) and the live
  // tracking map — optional at creation, but dispatch will refuse to use
  // this hospital as an origin/destination until it's set.
...
export default function HospitalsPage() {
  ...
  const canManage = hasPermission(PERMISSIONS.HOSPITAL_MANAGE);
  ...
  const toggleStatus = async (hospital: Hospital) => {
    if (!canManage) return; // defense in depth — server also enforces this
  ...
  return (
    <div className="p-8">
      <PageHeader
        title="Hospitals"
        subtitle={canManage ? "Manage hospital facilities and their departments" : "Hospitals in the network"}
        action={
          canManage ? (
      ...
      <form onSubmit={handleSearch} className="mb-4 flex gap-2">
        ...
        />
        <button type="submit" className="rounded-lg border border-slate-300 dark:border-slate-600 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">
          Search
        </button>
      </form>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {isLoading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400 dark:text-slate-500">No hospitals yet.</td></tr>
            ) : (
              items.map((h) => (
                <tr key={h.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  ...
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{h.code}</td>
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{h._count?.departments ?? 0}</td>
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{h._count?.users ?? 0}</td>
    ...
    <Modal title="Add hospital" onClose={onClose}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3">
          <Field label="Latitude (optional)" type="number" step="any" placeholder="18.5204" error={errors.latitude?.message} {...register("latitude")} />
          <Field label="Longitude (optional)" type="number" step="any" placeholder="73.8567" error={errors.longitude?.message} {...register("longitude")} />
        </div>
        <p className="-mt-3 mb-4 text-xs text-slate-400 dark:text-slate-500">
          Needed to dispatch ambulances to/from this hospital and to place it on the live tracking map. Can be added later.
        </p>
